...
def remaining_craters_matching(all_CW_params, all_CW_conic, all_CW_Hmi_k,
                               ...
                               CC_conics,
                               all_ID, matched_ids, param_to_id, id_reproject, id_ga):
    ...
    if not(len(fil_ncid) == 0):
        ...
        pairwise_ga, reproject = compute_pairwise_ga(remaining_CC_params, fil_ncid, all_CW_conic, all_CW_Hmi_k, est_cam)
        ...
        for j in range(len(remaining_indices)):
            if pairwise_ga[j, nearest_neighbors_idx[j]] < 2.1:
    ...
    return matched_ids, num_matches, param_to_id, id_reproject, id_ga
...
def reprojection_test_w_pnp_solver(des_tree, descriptors, c_ids, ID,
                      ...
                      CC_params, ordered_CC_params,
                      comb_id, matched_ids, top_n, matching_percentage, 
                      param_to_id, id_reproject, id_ga, CC_conics):
    ...
    for index in indexes:
        ...
        if np.sum(matched) == 3:
            ...
            matched_ids, num_matches, param_to_id, id_reproject, id_ga = remaining_craters_matching(db_CW_params, db_CW_conic, db_CW_Hmi_k,
                                                                  est_cam, cam_in_world_coord,
                                                                  CC_params, CC_conics,
                                                                  ID, matched_ids, param_to_id, id_reproject, id_ga)
            if ((num_matches) > (matching_percentage * CC_conics.shape[0])):  
                return True, est_cam, cam_in_world_coord, matched_idx, matched_ids, param_to_id, id_reproject, id_ga

    return False, est_cam, cam_in_world_coord, matched_idx, matched_ids, param_to_id, id_reproject, id_ga
...
def plot_reprojection(params_to_id, id_reproject, id_ga, image_name):
    ...
    print("Params_to_id,id_reproject,id_ga")
    print(len(params_to_id),len(id_reproject),len(id_ga))
    ...
    for key in params_to_id.keys():
        ...
        image = cv2.putText(image, str(id_ga[key]), (int(param[0]), int(param[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    for key in params_to_id.keys():
        ...
        data[key] = {'original': param, 'reprojected': reproject, 'ga': id_ga[key]}
...
def process(curr_img_params,img_name,i,CW_params, CW_conic, CW_conic_inv, CW_ENU, CW_Hmi_k, ID, crater_center_point_tree,CP_tree,CP_c_ids,matching_percentage):
    ...
    for comb_id in cc_comb:
        ...
        if CP_des is not None and not np.any(np.isnan(CP_des)):
            if solver == 'pnp':
                cp_match_flag, est_cam, cam_pos, matched_idx, matched_ids, param_to_id, id_reproject, id_ga = reprojection_test_w_pnp_solver(CP_tree, CP_des,
                                                                                                CP_c_ids,
                                                                                                ID,
                                                                                                CW_conic,
                                                                                                CW_Hmi_k,
    ...
    plot_reprojection(param_to_id, id_reproject, id_ga, img_name)
